...
public class Encriptado {
	public String samuelcamargo(String frase) {
		...
		String[] abecedario = { "a", "b", "c", "d", "e", "f", "g", "h", "i", "j", "k", "l", "m", "n", "o", "p", "q",
				"r", "s", "t", "u", "v", "w", "x", "y", "z" }; // se inicializa un vector para contener las letras del
																// abecedario
		String[] deletreo = new String[frase.length()]; // se inicializa un vector para almacenar la palabra a encriptar
		for (int i = 0; i < frase.length(); i++) { // para i = 0 mientras i sea menor que el largo de la palabra i
													// aumenta en 1

			deletreo[i] = frase.substring(i, 1 + i); // se toma la palagra ingresada y se separa letra por letra

		}
		for (int j = 0; j < deletreo.length; j++) { // para j = 0 mientras j sea menor que el largo del deletreo de la
													// palabra aumentar j en 1
			...
			for (int i = 0; i < abecedario.length; i++) { // para i = o mientras i sea menor que 26 aumentar i en 1
				...
				int condicion = i - frase.length() * 2; /*
														 * se inicializa una variable que almacena las condiciones de
														 * encriptacion
														 * que es restar el numero de espacios del largo de la palabra y
														 * se multiplica ese valor en 2
														 */

				if (condicion < 0) { // se crea una condicion para evitar que la encriptacion se desborde del
										// abecedario
	...
	public String sergio(String frase) {
		String[] vector = { "a", "b", "c", "d", "e", "f", "g", "h", "i", "j", "k", "l", "m", "n", "ñ", "o", "p", "q",
				"r", "s", "t", "u", "v", "w", "x", "y", "z", " " };
		String resultado = "";
		for (int i = 0; i < frase.length(); i++) {// ciclo desde la primera letra hasta la ultima
			for (int x = 0; x < vector.length; x++) {// ciclo de a hasta z
				if (vector[x].equals(frase.substring(i, i + 1))) {// comprueba si la letra es igual a la posicion en el
																	// vector
					int y = x;
	...
	public String angelapeña(String frase) {
		...
		for (int contador = longitud_palabra; contador > 0; contador--) {
			...
			resultante1 = resultante1 + letra; // agrego la letra en que va el ciclo a la variable de la palabra al
												// revés
		}

		/*
		 * ahora defino un vector de caracteres especiales o signos que voy a utilizar
		 * para encriptar segun la posicion del abecedario.
		...
		String[] abecedario = { "a", "b", "c", "d", "e", "f", "g", "h", "i", "j", "k", "l", "m", "n", "o", "p", "q",
				"r", "s", "t", "u", "v", "w", "x", "y", "z", };
		String palabra_encriptada = ""; // defino una variable vacia, tipo string en la que voy a meter la palabra
	...
	public String yirman (String frase, int año, int mes) {
		/*
		 * esta funcion recibe una palabra para encriptar a su vez recibe un año
		 * y un mes (prferiblemente año y ,es de nacimiento)que mediente operaciones
		 * matematicas basica
		 * genera un numero personal, el cual sera el numero de veces que avanza de la
		 * posicion original a la nueva posicion
		 * s
		 ...
		 */
		String[] abc = { "array", "bucle", "cadena", "definir", "enlace", "funcion", "gato", "hijo", "incremento",
				"java", "kilo", "lamento", "matriz", "nada", "ñandu", "operador", "patron", "quizz", "raton", "sistema",
				"terminal", "ubicacion", "vida", "wilslson", "xilofon", "yuca", "zzz" };
		String palabra = frase;
		/*
		 * el siguiente proceso se hace para tener un numero personalizado depentienndo
		 * el año y mes de nacimiento
		 */
		int codigo = (año * mes) / (mes + (año / 2));// genera un numero realizando operaciones matematicas basado en
														// los datos recibidos
		...
		/*
		 * for que sirve para aumentar la variable dependiendo el numero de las letras
		...
		for (int incremento = 0; incremento <= palabra.length() - 1; incremento++) {
			for (int posicion = 0; posicion <= abc.length - 1; posicion++) { // for para recorrer el arreglo
				...
				if (palabra.substring(incremento, incremento + 1).equals(letra)) {
					...
					/*
					 * esta condicion sirve para que cuando la letra encriptada supere el tamaño del
					 * arreglo esta
					...
					if (posicion2 > abc.length) {
						 ...
						 * con el codigo que cree anteriormente que es el que me dice las veces que
						 * avanzo de legtra
						 * en el arreglo, luego lo resto con la longitud del arreglo y el resultado es
						 * el
						 * numero que veces que avnzara ahora desde el primer caracter del arreglo
						 */
						int volver = (posicion2) - abc.length;
						/*
						 * agrega palabras a las ya guardadas, ademas de un "/" para distinguir mejor
						 * cada palabra
						 */
	...
	public String nicolcepeda(String frase) {
		 ...
		 */
		String[] letraEsp = { "A", "B", "C", "D", "E", "F", "G", "H", "I", "J", "K", "L", "M", "N", "Ñ", "O", "P", "Q",
				"R",
				"S", "T", "U", "V", "W", "X", "Y", "Z" };
		String[] letraMorse = { ".-", "-...", "-.-.", "-..", ".", "..-.", "--.", "....", "..", ".---", "-.-", ".-..",
				"--",
	...
	public String lauracorrea(String palabra) {
		...
		String[] lista_palabras =
				...
				{ "magdalena", "fishburne", "crucial", "kendrik", "suena", "perfume", "goodgirls", "bahia", "swift",
						"semejante", "akkas", "ambulance", "tiempos", "mango", "ghost", "zepet", "buque", "world",
						"misma", "anestecia", "uchuvas", "alvin", "williamwilson", "boxeo", "nsync", "gozar" };
	...
	public String tomasespinel(String palabra, int numero, boolean desicion) {// creamos metodo para encriptar la
																				// palabra
		int posicion = 0;
	...
	public String juanfuquene(String frase) {
		...
		for (int interno = 0; interno < longitudPalabra; interno++) { // Ciclo "para" para recorrer cada letra de la
			...
			String letraComp = palabraEncriptar.substring(interno, interno + 1); // El String letraComp almacenará cada
																					// letra de la palabra que tome en
																					// ese
																					// momento
	...
	public String samuelcastro(String palabra) {
		...
		String palabranueva = "";
		// letra nueva la cual va a ser la nueva letra que va a formar la palabra
		// encriptada
		String letranueva = "";
		...
		for (int i = 0; i < longifrase; i++) {
				...
				if (palabra.substring(i, hasta).compareTo(espacio) == 0) {
					...
					if (palabra.substring(i, hasta).compareTo(vector[q]) == 0) {
						...
						if (hasta % 2 == 0) {
							...
							for (int w = 1; w < 27; w++) {
								// si la posicion en el abecedario de la letra es igual a w(letra que recorre el
								// abecedario) hacer
								if (numeroletra == w) {
			...
			// aqui se va formando la palabra con las letras que nos arroja los
			// condicionales
	...
	public String samirYate(String frase) {
		// Creo la clase Scanner y la importo para que nos permite obtener la entrada de
		// datos por teclado:
	...
	public static String EncriptarDavid(String frase) {
		/*
		...
		if (frase.length() % 2 == 0) {// Se procesa cuando el numero de palabras ingresadas es par
			...
			for (int i = 0; i < frase.length(); i++) { // Se imprimen los números pares correspondientes a la palabra
			...
			for (int i = 0; i < frase.length(); i++) { // Se imprimen los números impares correspondientes a la palabra
				char caracter = frase.charAt(i);
